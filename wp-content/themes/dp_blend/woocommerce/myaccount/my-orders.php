	...
	<table class="shop_table my_account_orders">
		...
		<tbody><?php
			foreach ( $customer_orders as $customer_order ) {
				...
				?><tr class="order">
					<td class="desktop-hidden"><?php __( 'Order', 'woocommerce' ); ?></td><td class="order-number">
						<a href="<?php echo esc_url( add_query_arg('order', $order->id, get_permalink( woocommerce_get_page_id( 'view_order' ) ) ) ); ?>">
							<?php echo $order->get_order_number(); ?>
						</a>
					</td>
					<td class="desktop-hidden"><?php __( 'Date', 'woocommerce' ); ?></td><td class="order-date">
					...
					<td class="desktop-hidden"><?php __( 'Status', 'woocommerce' ); ?></td><td class="order-status" style="text-align:left; white-space:nowrap;">
						<?php echo ucfirst( __( $status->name, 'woocommerce' ) ); ?>
					</td>
					<td class="desktop-hidden"><?php __( 'Total', 'woocommerce' ); ?></td><td class="order-total">
