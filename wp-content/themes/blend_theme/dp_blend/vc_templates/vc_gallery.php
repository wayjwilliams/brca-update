...
if ( $type == 'nivo' || $type == 'nivo_thumb') {
	if ( $type == 'nivo') $slides_wrap_start = '<div class="nivoSlider">';
	if ( $type == 'nivo_thumb') $slides_wrap_start = '<div class="nivoSlider_thumb">';
	$slides_wrap_end = '</div>';
...
foreach ( $images as $attach_id ) {
	$i ++;
	if ( $attach_id > 0 ) {
		$post_thumbnail = dp_getImageBySize( array( 'attach_id' => $attach_id, 'thumb_size' => $img_size ) );
	} else {
...
$output .= '<div class="wpb_gallery_slides' . $type . ' '.$navigation.'" data-interval="' . $interval . '"' . $flex_fx . '>' . $slides_wrap_start . $gal_images . $slides_wrap_end . '</div>';
