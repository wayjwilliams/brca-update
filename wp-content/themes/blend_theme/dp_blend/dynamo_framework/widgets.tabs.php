...
class DP_Tabs_Widget extends WP_Widget {
	 ...
	 *
	 **/
	function DP_Tabs_Widget() {
		$this->WP_Widget(
			'widget_dp_tabs', 
			__( 'DP Tabs', DPTPLNAME ), 
			array( 
				'classname' => 'widget_dp_tabs', 
				'description' => __( 'Use this widget to show tabs created form the selected sidebar', DPTPLNAME) 
			)
		);
	...
	function widget($args, $instance) {		
		...
		if ($sidebar !== '') {
			...
			echo '<div class="dp-tabs" data-event="'.$event.'" data-autoanim="'.$autoanim.'" data-speed="'.$anim_speed.'" data-interval="'.$anim_interval.'">';
			echo '<div class="dp-tabs-wrap">';
			...
			foreach($widget_code as $code) {
				...
				if(count($title_match) > 1 && isset($title_match[1][0])) {
					...
					array_push($tabs, _e('No title specified!', DPTPLNAME));
				}
				
				array_push($tabs_content, preg_replace('@{TABS_TITLE}(.*?){TABS_TITLE}@mis', '', $code));
			}
			// generate the tabs content
			echo '<ol class="dp-tabs-nav">';
			for($i = 0; $i < count($tabs); $i++) {
				echo '<li'.(($i == 0) ? ' class="active"' : '').'>' . $tabs[$i] . '</li>';
			}
			echo '</ol>';
			
			echo '<div class="dp-tabs-container">';
			for($i = 0; $i < count($tabs_content); $i++) {
				echo '<div class="dp-tabs-item'.(($i == 0) ? ' active' : '').'">' . $tabs_content[$i] . '</div>';
			}
			echo '</div>';
	...
	?>
		...
		<p>
			<label for="<?php echo esc_attr( $this->get_field_id( 'sidebar' ) ); ?>"><?php _e( 'Tabs source:', DPTPLNAME ); ?></label>
		...
		<p>
			<label for="<?php echo esc_attr($this->get_field_id('event')); ?>"><?php _e('Tabs activator event:', DPTPLNAME); ?></label>
			...
			<select id="<?php echo esc_attr( $this->get_field_id('event')); ?>" name="<?php echo esc_attr( $this->get_field_name('event')); ?>">
				<option value="click"<?php echo (esc_attr($event) == 'click') ? ' selected="selected"' : ''; ?>>
					<?php _e('Click', DPTPLNAME); ?>
				</option>
				<option value="hover"<?php echo (esc_attr($event) == 'hover') ? ' selected="selected"' : ''; ?>>
					<?php _e('Hover', DPTPLNAME); ?>
				</option>
			</select>
		</p>
		
		<p>
			<label for="<?php echo esc_attr($this->get_field_id('autoanim')); ?>"><?php _e('Auto animation:', DPTPLNAME); ?></label>
			...
			<select id="<?php echo esc_attr( $this->get_field_id('autoanim')); ?>" name="<?php echo esc_attr( $this->get_field_name('autoanim')); ?>">
				<option value="enabled"<?php echo (esc_attr($autoanim) == 'enabled') ? ' selected="selected"' : ''; ?>>
					<?php _e('Enabled', DPTPLNAME); ?>
				</option>
				<option value="disabled"<?php echo (esc_attr($autoanim) == 'disabled') ? ' selected="selected"' : ''; ?>>
					<?php _e('Disabled', DPTPLNAME); ?>
				</option>
			</select>
		</p>
		
		<p>
			<label for="<?php echo esc_attr( $this->get_field_id( 'anim_speed' ) ); ?>"><?php _e( 'Animation speed (ms):', DPTPLNAME ); ?></label>
		...
		<p>
			<label for="<?php echo esc_attr( $this->get_field_id( 'anim_interval' ) ); ?>"><?php _e( 'Animation interval (ms):', DPTPLNAME ); ?></label>
