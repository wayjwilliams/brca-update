...
class DP_Comments_Widget extends WP_Widget {
	...
	function widget($args, $instance) {
		...
		$title = apply_filters('widget_title', empty($instance['title']) ? _e( 'Latest Comments', DPTPLNAME ) : $instance['title'], $instance, $this->id_base);
		...
		if (count($comments)) {			
			if(count($comments) > 0) {
				...
				for($i = 0; $i < count($comments); $i++) {
					echo '<li'.(user_can($comments[$i]->user_id, 'delete_users') ? ' class="dp-admin-comment"' : '').'>';
